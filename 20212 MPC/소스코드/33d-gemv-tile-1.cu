...
__global__ void kernelGEMVtransposed( float* Z, const float* Atr, const float* X, const float* Y,
	...
	if (gy < matsize) {
		...
		register unsigned ntiles = (matsize + BLOCK_SIZE - 1) / BLOCK_SIZE;
		for (register unsigned tile = 0; tile < ntiles; tile++) {
			register nelem = min(BLOCK_SIZE, remaining);
			remaining -= BLOCK_SIZE;
			if (threadIdx.y < nelem) {
				register unsigned idxX = tile *  BLOCK_SIZE + threadIdx.y;
				s_X[threadIdx.y] = X[idxX];
			}
