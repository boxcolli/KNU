...
static char* vsSource = "#version 130 \n\
	...
	void main(void) { \n\
  		gl_Position = um_persp*um_view*utrans_z*urotate*uscale*utrans_o*aPosition; \n\
  	vColor = aColor; \n\
}";
...
void copy3(float* dest, float* src) {
	dest[0] = src[0];
	dest[1] = src[1];
	dest[2] = src[2];
}
void myinit(void) {
	...
	colors = (GLfloat*)malloc(num_vertices * 4 * sizeof(GLfloat));
	GLfloat pallette[24];
	pallette[0] = 50.0/255.0; pallette[1] = 14.0/255.0; pallette[2] = 59.0/255.0;
	pallette[3] = 76.0/255.0; pallette[4] = 42.0/255.0; pallette[5] = 133.0/255.0;
	pallette[6] = 107.0/255.0; pallette[7] = 127.0/255.0; pallette[8] = 215.0/255.0;
	pallette[9] = 188.0/255.0; pallette[10] = 237.0/255.0; pallette[11] = 246.0/255.0;
	pallette[12] = 221.0/255.0; pallette[13] = 251.0/255.0; pallette[14] = 210.0/255.0;
	pallette[15] = 228.0/255.0; pallette[16] = 217.0/255.0; pallette[17] = 255.0/255.0;
	pallette[18] = 41.0/255.0; pallette[19] = 82.0/255.0; pallette[20] = 74.0/255.0;
	pallette[21] = 136.0/255.0; pallette[22] = 128.0/255.0; pallette[23] = 152.0/255.0;
	for (i = 0; i < num_vertices; i++) {
		GLfloat x = vertices[i*4+0];
		GLfloat y = vertices[i*4+1];
		GLfloat z = vertices[i*4+2];
		if (x > 0) {
			if (y > 0) {
				if (z > 0) {
					copy3(colors+i*4, pallette+0);
				}else {
					copy3(colors+i*4, pallette+3);
				}
			}else {
				if (z > 0) {
					copy3(colors+i*4, pallette+6);
				}else {
					copy3(colors+i*4, pallette+9);
				}
			}
		}else {
			if (y > 0) {
				if (z > 0) {
					copy3(colors+i*4, pallette+12);
				}else {
					copy3(colors+i*4, pallette+15);
				}
			}else {
				if (z > 0) {
					copy3(colors+i*4, pallette+18);
				}else {
					copy3(colors+i*4, pallette+21);
				}
			}
		}
		colors[i*4+3] = 1.0;
